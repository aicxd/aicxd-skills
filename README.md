...
一个只读的系统环境自查工具。触发后跑 `scan.ps1` 扫描本机，生成一份结构化的《Claude / Codex 系统环境自查报告》，把你当前环境里所有被 Anthropic 用于识别用户环境的信号全部列出来，逐项给出判定和调整方案。**脚本全程只读、默认离线、不上传任何数据**（单文件可当场审计；仅 `-CheckIP` 才联网查一次出口 IP）。
