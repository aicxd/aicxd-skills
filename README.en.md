...
| Skill | What it does |
|---|---|
| 🛡️ [**claude-ban-guard**](#️-claude-ban-guard) | Generates a Claude / Codex system environment report — checks 6 dimensions (timezone, proxy, network, browser) and gives ✅/⚠️/❌ verdicts with adjustment steps |
...
> *"Anthropic embedded a local environment signal detection layer in Claude Code that classifies your environment before your request goes out. This Skill scans which signals your machine currently triggers and gives step-by-step guidance to adjust each one."*
...
| 6 | Account resilience | Whether a DeepSeek fallback key is in place as a backup if primary Claude access fails |

**Report format**

Each item gets a ✅ / ⚠️ / ❌ verdict, labeled with 🤖 Claude can handle it or ✋ manual action needed. Reports current signal state as-is.

**How to trigger**

```
system environment check
check my local Claude config
check if my timezone/BASE_URL is exposed
/claude-ban-guard
```

**🖥️ Fully tested on Windows, works on macOS (requires pwsh)**

→ [SKILL.md](./claude-ban-guard/SKILL.md) · [Steganographic marking explained](./claude-ban-guard/README.md)
